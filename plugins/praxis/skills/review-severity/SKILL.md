...
description: One severity model for code review — Critical/Important/Minor by consequence-if-shipped (impact × reachability), with confidence as a separate report gate.
...
**Severity = consequence-if-shipped = impact × reachability.** Confidence is *not* part
of it (separate gate, below).

- **Impact** = KIND (corruption / breach / crash / wrong-result / disclosure) × MAGNITUDE
  (damage per occurrence) × TIME-HORIZON (irreversibility is just long-horizon impact).
- **Reachability** = P(path reached) × P(trigger), classed **live / edge / dead**. Judge it
...
micro-opt, an O(n²) loop over a provably-and-enforced tiny bound), chosen by the
...
| O(n²) over a provably-bounded tiny list | Minor | reachability high, impact ~0 |
