...
3. **Index repeated comments** -- a retelling is invisible from the file it sits in, so run `find-duplicate-comments.js`. It ships with praxis, not the repo under review, and `CLAUDE_PLUGIN_ROOT` is unset for Bash: search `~/.claude/plugins/cache/*/praxis/*/hooks/scripts/` (highest version) as well as the working tree. Say so in your output if you cannot run it -- silence there reads as "no retellings found".
